...
public class Updat3r {
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				String saveFilePath = new File(
						Main.getInstance().getClass().getProtectionDomain().getCodeSource().getLocation().toURI()).getPath();
	...
	public void sendUpdateMessage(Player p) {
		...
		p.sendMessage("§3You're now using version §b" + Main.getInstance().getDescription().getVersion() + "§3.");
	...
	public class Update {
		...
		public boolean isNewer() {
			...
			String[] sdbver = Main.getInstance().getDescription().getVersion().split("\\.");
